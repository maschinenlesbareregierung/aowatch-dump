const electionProgramList = require('@malereg/aowatch-client/entities/entity.election-program').electionProgramList;
...
module.exports =  async function loadElectionProgram() {
    // create a new progress bar instance and use shades_classic theme
    const bar1 = new cliProgress.SingleBar({
        format: 'Electionprogram |' + '{bar}' + '| {percentage}% || {value}/{total} Chunks',
    ...
    const res = listAll(electionProgramList, 4, politicianEmitter);
    return res.then((res)=>{
        bar1.stop();
        return res;
    });
